...
<div id="gameArea">
    ...
    <!-- For displaying the in game timer -->
	<span id="counter"></span>
	<!-- For displaying the input textbox for scoreboard -->
	<span id="text" hidden>
...
<script>
	...
	/*Loads the mouse click function */
	...
	/* Functions that are ran onload of screen */
        ...
        function updateTimer() {
            ...
            if (gameState == true) {
                ...
                if (livesCount == 0) {
					...
					//Movement of the bubbles only if gameState is true
					if (gameState == true) {
                        timerHandle = setTimeout(updateTimer, time.getUTCMilliseconds() + 500);
                    }
					
					//Stops the movement of bubbles if gameState is false
                    if (gameState == false) {
	...
	//Plays music or mutes music depending on original state
    function music() {
		...
		//Unmutes if it was original muted
        if (playMusic == false) {
    ...
    var drawStartScene = function () {
        ...
        //Creates the start button and sets the function on click
		...
		//Creates the scoreboard button and sets the function on click
        var btn2 = new Button({
            x: canvas.width / 2 - 65,
            y: 380,
            image: scoreButton,
		...
		//Determines if buttons are clicked
        window.onclick = function () {
                if (start.clicked()) {
                    start.click();
                }
	...
	/* Resets all values back to their original state when restart of game */
    ...
    var failStage = function () {
		...
		//Determines that the buttons are clicked
        window.onclick = function () {
            if (btn1.clicked()) {
                btn1.click();
            }
	...
	/* Draws the final screen when you clear the game */
    var youWin = function () {
        notClickable();
        if (levels == 3) {
            totalScore = level3Score;
		...
		//Determines if the buttons are clicked
        window.onclick = function () {
            if (btn1.clicked()) {
                btn1.click();
            }
